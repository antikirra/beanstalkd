...
//   1. x86 with SSE4.2  — hardware CRC32 instruction (Nehalem 2008+),
//      selected at compile time.
//   2. AArch64 with CRC32 extension (ARMv8.1+ / most ARMv8.0 chips) —
//      __crc32c* ACLE intrinsics, selected at RUNTIME via
//      getauxval(AT_HWCAP) & HWCAP_CRC32. Baseline aarch64 compilers
//      (Debian gcc defaults to armv8.0) do not define
//      __ARM_FEATURE_CRC32, so a compile-time-only switch silently
//      falls back to the software table even on hardware that has the
//      instruction — that fallback cost ~67% of all CPU on 16KB-body
//      PUTs (S3 benchmark regression, perf-profiled).
//   3. Portable table-driven software fallback (also the runtime
//      fallback for aarch64 chips without the CRC32 extension).
// All three compute the identical CRC32C; the known-answer test
// CRC32C("123456789") == 0xE3069283 (the CRC-32/ISCSI check value)
// is enforced by cttest_crc32c_known_vector in testcore.c.

// Table-driven software fallback. The table maps byte b to the
// CRC32C of that byte: t = b; for 8 bits: t = (t>>1) ^ (0x82F63B78
// if t&1). Written out as a constant so the CRC works identically
// before any runtime init and under -flto.
...
__attribute__((hot))
static uint32
wal_crc32c_sw(uint32 crc, const void *buf, size_t n)
{
...
// so on that path we target it unconditionally — no runtime CPUID
// dispatch. The Makefile adds -msse4.2 for this file on x86_64 only.
__attribute__((hot))
uint32
...
wal_crc32c_hw(uint32 crc, const void *buf, size_t n)
{
    ...
    // Same 8-bytes-per-iteration shape as the SSE4.2 path.
    // __crc32cd operates on the low 32 bits of a 64-bit value.
    while (n >= 8) {
        uint64_t v;
        __builtin_memcpy(&v, p, 8);
        crc = CRC32C_D(crc, v);
...
wal_crc32c(uint32 crc, const void *buf, size_t n)
{
    // glibc caches the auxv value; cheap per call. HWCAP_CRC32 is set
    // on every chip with the ARMv8 CRC32 extension (ARMv8.1+ and most
    // ARMv8.0). Chips without it keep the table path — no SIGILL.
    if (getauxval(AT_HWCAP) & HWCAP_CRC32)
        return wal_crc32c_hw(crc, buf, n);
    return wal_crc32c_sw(crc, buf, n);
}
